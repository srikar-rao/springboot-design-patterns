...
/**
 * Concrete implementation of LoanProcessor for business loans.
 */
public class BusinessLoanProcessor implements LoanProcessor {
    ...
    public LoanStatus handle(LoanStatus.Submitted submitted) {
        ...
        return new LoanStatus.Denied("Validation failed for business loan.");
    }

    @Override
    public LoanStatus handle(LoanStatus.Reviewed reviewed) {
        double interestRate = interestRateStrategy.calculate(reviewed.request());
        double approvedAmount = reviewed.request().getRequestedAmount() * 0.7; // Approve 70%
